...
/// Staged KNN search tables for OctVoxMap.
/// Source: Super-LIO (RA-L 2026), adapted for DLIO.

/// The last number is unused (sentinel).
inline constexpr std::array<float, 6> orders_min_dis = {
  0.250000, 0.353553, 0.500000, 0.559017, 0.612372, 10
};
inline constexpr std::array<float, 6> orders_min_dis2 = {
  0.062500, 0.125000, 0.250000, 0.312500, 0.375000, 100
};

inline const std::array<Eigen::Vector3i, 60> HKNN_neighbor_voxel = 
{
...
inline constexpr std::array<uint16_t, 7> flat_search_order_offsets = {{
  0, 43, 135, 219, 321, 465, 593,
}};

inline constexpr std::array<uint8_t, 593> flat_search_order = {{
  // Group0
  0, 8, 0, 1, 2, 3, 4, 5, 6, 7, 
  1, 4, 2, 3, 6, 7, 
  2, 4, 4, 5, 6, 7, 
